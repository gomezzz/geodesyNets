...
def bind_label(method: str, use_acc: bool = True, **kwargs) -> Callable[[torch.Tensor], torch.Tensor]:
    ...
    Keyword Args:
        ...
        low_resolution (bool): loads the low resolution polyhedral mesh if given and set to true
    ...
    if method == 'mascon':
        ...
        if sample := kwargs.get("sample", None):
            sample = f"{sample}_lp" if kwargs.get("low_resolution", False) else sample
